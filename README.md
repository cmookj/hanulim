...
│   ├── HNInputContext.swift        한글 조합 상태 기계
...
이 옵션을 활성화하면 한울림 한글 모드(두벌식, 세벌식 등)에서 ESC 키를 누를 때 자동으로 로마자 모드로 전환됩니다. vi/vim에서 입력 모드를 빠져나올 때 동시에 입력기도 로마자로 전환되므로, 명령 모드에서 한글이 입력되는 불편함을 없애줍니다.

**동작 조건:**
- 현재 입력 소스가 한울림 한글 모드(로마자 모드 제외)일 것
...
ESC 이벤트는 소비하지 않고 항상 앱에 그대로 전달됩니다. 한울림은 ESC를 관찰하는 동시에 TIS 입력 소스 전환을 부수 효과로 수행합니다. vi/vim은 ESC를 정상적으로 수신하여 입력 모드에서 명령 모드로 전환하고, 한울림도 동시에 로마자 모드로 전환됩니다.

이 기능은 `HNEventTap`(CGEventTap 수준)에서 처리되므로 터미널 에뮬레이터를 포함한 모든 앱에서 동작합니다.
